...
def main():
    ...
    if args.command == 'predict':
        if args.webcam:
            from .monoloco.visuals.webcam import webcam
            webcam(args)
        else:
            if args.social:
                from .monoloco import predict_social
                predict_social.predict(args)
            else:
                from .monoloco import predict
                predict.predict(args)

    elif args.command == 'prep':
        if 'nuscenes' in args.dataset:
            from .monoloco.prep.preprocess_nu import PreprocessNuscenes
            prep = PreprocessNuscenes(args.dir_ann, args.dir_nuscenes, args.dataset, args.iou_min)
            prep.run()
        if 'kitti' in args.dataset:
            from .monoloco.prep.preprocess_ki import PreprocessKitti
            prep = PreprocessKitti(args.dir_ann, args.iou_min)
            prep.run()

    elif args.command == 'train':
        from .monoloco.train import HypTuning
        if args.hyp:
